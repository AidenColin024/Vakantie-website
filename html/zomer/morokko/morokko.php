...
$sql = "SELECT hotel_naam, link FROM hotels WHERE name = 'Morokko' AND hotel_naam != ''";
$stmt = $conn->prepare($sql);
$stmt->execute();
$morokkoHotels = $stmt->fetchAll(PDO::FETCH_ASSOC);

if (!empty($_GET['hotelSearch'])) {
    $zoekHotel = $_GET['hotelSearch'];

    $stmt = $conn->prepare("SELECT link FROM hotels WHERE name = 'Morokko' AND hotel_naam = :hotelnaam LIMIT 1");
    $stmt->execute([':hotelnaam' => $zoekHotel]);
    $hotelData = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($hotelData && !empty($hotelData['link'])) {
        header("Location: /zomer/morokko/" . $hotelData['link']);
        exit;
    } else {
...
<head>
    ...
    <title>Zomervakantie Morokko - Polar & Paradise</title>
...
<main class="pp-content">
    <div class="page-content">
        ...
        <section class="destination-blocks">
            ...
            $stmt->execute([':land' => 'Morokko']);
            $hotels = $stmt->fetchAll(PDO::FETCH_ASSOC);

            if (empty($hotels)) {
                echo "<p style='margin-left:1rem;'>Er zijn nog geen hotels toegevoegd voor Morokko.</p>";
            } else {
