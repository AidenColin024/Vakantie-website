...
$sql = "SELECT hotel_naam, link FROM hotels WHERE name = 'Spanje' AND hotel_naam != ''";
$stmt = $conn->prepare($sql);
$stmt->execute();
$spanjeHotels = $stmt->fetchAll(PDO::FETCH_ASSOC);

if (!empty($_GET['hotelSearch'])) {
    $zoekHotel = $_GET['hotelSearch'];

    $stmt = $conn->prepare("SELECT link FROM hotels WHERE name = 'Spanje' AND hotel_naam = :hotelnaam LIMIT 1");
    $stmt->execute([':hotelnaam' => $zoekHotel]);
    $hotelData = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($hotelData && !empty($hotelData['link'])) {
        header("Location: /zomer/spanje/" . $hotelData['link']);
        exit;
    } else {
...
<head>
    ...
    <title>Zomer Vakantie Spanje - Polar & Paradise</title>
...
<section class="vakantie zomer-hero">
    <img src="../../images/OIP4.jpg" alt="Zonvakantie Spanje" class="hero-img" />
    <div class="hero-text">
        <h1>Ontdek jouw droomvakantie in Spanje</h1>
        <p>Geniet van zon, zee en cultuur in het prachtige Spanje. Bekijk onze zorgvuldig geselecteerde hotels!</p>

        <form method="GET" action="" id="hotelSearchForm">
            <label for="hotelSearch">Zoek een hotel:</label><br>
            <input list="hotelsList" id="hotelSearch" name="hotelSearch" placeholder="Typ hotelnaam...">
            <datalist id="hotelsList">
                <?php foreach ($spanjeHotels as $hotel): ?>
...
<main class="pp-content">
    <div class="page-content">
        <aside class="pp-filters">
            <h3>Filter jouw Zomervakantie</h3>
            ...
            <select id="region" name="region">
                ...
                <option>Baleares (Mallorca, Ibiza)</option>
            </select>
            ...
            <label><input type="checkbox"> Zwembad aanwezig</label>
        </aside>
