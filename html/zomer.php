...
<script>
    function redirectToLandZomer() {
        ...
        const landen = ['spanje', 'griekenland', 'morokko', 'turkije'];

        if (landen.includes(land)) {
            window.location.href = `/zomer/${land}/${land}.php`;
        } else {
            alert("Land niet gevonden. Mogelijke opties: Spanje, Griekenland, Morokko, Turkije.");
        }
